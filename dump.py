...
while not QQ_PID:
    ...
    for pid in psutil.pids():
        try:
            ...
            print(p.cmdline())
            # 检查进程名称是否为 "QQ.exe"、命令行参数是否多于一个，且路径中不包含 "QQNT"
            if p.name() == "QQ.exe" and len(p.cmdline()) > 1 and "QQNT" not in p.exe():
                QQ_PID = pid  # 记录符合条件的进程 PID
                break  # 找到符合条件的进程后退出循环
        except:
            # 忽略无法访问的进程
            continue

    if not QQ_PID:
        print("未检测到qq进程, 等待qq进程启动中")
        time.sleep(0.1)

print("QQ pid is:", QQ_PID)

hook_script = """
// 将二进制缓冲区转换为十六进制字符串表示，用于调试输出内存内容。
...
//加载KernelUtil.dll模块，并通过模式扫描查找特定函数的地址。如果找到多个或没有找到，发送相应的消息并退出。
...
// 通过模式扫描找到关键函数的地址，包括key_function、open_function、name_function、rekey_function、close_function和exec_function。
...
// 初始化一些变量和内存分配，包括目标密钥长度、数据库名称、空密码、原始密码等
...
// 当调用key_function时，检查是否与目标数据库匹配。如果匹配，复制密码并进行数据库解密和重新加密操作。并输出调试信息。
...
// 当调用rekey_function时，检查是否与目标数据库匹配，并输出调试信息
